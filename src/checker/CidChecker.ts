...
export default class CidChecker {
  ...
  private calculateAvgProviderScore (matchedRetrievability: Retrievability[]): number {
    const { totalClientDealCount, totalSuccessRate } = matchedRetrievability.reduce(
      (acc, { total_deal_size, success_rate }) => {
        acc.totalClientDealCount += total_deal_size
        acc.totalSuccessRate += success_rate * total_deal_size
        return acc
      },
      { totalClientDealCount: 0, totalSuccessRate: 0 }
    )

    if (totalSuccessRate === 0 || totalClientDealCount === 0) return 0
    return totalSuccessRate / totalClientDealCount
  }

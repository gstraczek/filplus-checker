...
const handler: ApplicationFunction = (
  app: Probot,
  ...
  if (_options.getRouter != null) {
    ...
    reportRoute.get('/', (req: Request, res: Response) => {
      ;(async () => {
        if (
          req.query.issue === undefined ||
        ...
        try {
          response = await checker.octokit.request(
      ...
      async (context) => {
        ...
        if (context.payload.action === 'labeled') {
          ...
          !context.payload.comment.body
            ...
            .startsWith(process.env.TARGET_COMMENT!)
        ) {
          return
        } else {
          const addresses = context.payload.comment.body
            .trim()
            .split(/\s+/)
            .slice(1)
            .filter((s) => s.match(/[0-9a-zA-Z]+/))
        ...
        const checker = getCidChecker(
          app.log.child({ contextId: context.id }),
          context.octokit
        )
        if (
          ...
          const pre: PullRequestReviewCommentCreatedEvent =
            context.payload as PullRequestReviewCommentCreatedEvent

          const result = await checker.checkFromPR(
            pre,
            criteria,
            otherAddresses
          )
        ...
        const icce: IssueCommentCreatedEvent =

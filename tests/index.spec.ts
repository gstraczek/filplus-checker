...
describe('checkerApp', () => {
  ...
  it('should post a new comment when triggered', async () => {
    ...
    const mock = nock('https://api.github.com')
      .post(
        '/repos/data-preservation-programs/filecoin-plus-large-datasets/issues/1/comments',
        (body: any) => {
    ...
    const mockChecker = jasmine.createSpyObj('CidChecker', {
      check: Promise.resolve(['test-summary', 'test-content'])
    })
    spyOn(dependency, 'getCidChecker').and.returnValue(mockChecker)
    process.env.TARGET_LABEL = 'bot:readyToSign'
    process.env.CRITERIA =
      '[{"lowReplicaThreshold": 1, "maxDuplicationPercentage": 1, "maxProviderDealPercentage": 1, "maxPercentageForLowReplica": 1}]'
